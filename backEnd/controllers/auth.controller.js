...
export const register = asyncHandler(async (req, res) => {
    ...
    const { user, accessToken } = await auth.handleRegister(name, email, password, number);

    res.status(200).json({ success: true, message: `Successfully Registered`, user, accessToken });
});
